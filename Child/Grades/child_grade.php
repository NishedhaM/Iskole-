...
<head>
    ...
    <title>Grades</title>
    <link rel="stylesheet" href="child_grade.css"/>
</head>
<body>
    ...
    <div class="nav-container">
      ...
      <h1 class="title" align="center">Grades</h1>
      <nav>
      ...
      if($c_date>=$subs){?>
       <div class="gr1">
        ...
        <input type="hidden" name="view_id" value="G1">
              <button type="submit" name="view_btn" class="button1"> <img src="../images/one.png" align="left" height="70px" width="40px">
              <h6>Grade 1</h6>
        </button></form></div> <?php
      }else{
 ...
 <div class="gr1">
        <form action="../Subject/child_subject.php" method="post">
        <input type="hidden" name="view_id" value="G1">
              <button type="submit" name="view_btn" class="button1" disabled> <img src="../images/one.png" align="left" height="70px" width="40px">
 ...
 <?php
  ...
  }else{        $sql="SELECT student_id FROM `paid_items` WHERE student_id=$_SESSION[id] AND gid='G2' AND valid_till>='$date'";
                ;
                $query_run = mysqli_query($conn, $sql);
                if($result = mysqli_query($conn,$sql)){
                     if(mysqli_num_rows($result) > 0){
 ...
 <?php
      ...
      }else{          $sql="SELECT student_id FROM `paid_items` WHERE student_id=$_SESSION[id] AND gid='G4' AND valid_till>='$date'";
                ;
                $query_run = mysqli_query($conn, $sql);
                if($result = mysqli_query($conn,$sql)){
                     if(mysqli_num_rows($result) > 0){
